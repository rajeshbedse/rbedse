...
def run(skip_phase1: bool = False, run_date: str | None = None, dry_run: bool = False) -> Path:
    ...
    # Regulation 31 is deliberately fetched after the shortlist exists so the
    # canonical output contains only symbols that can actually reach RYB. This
    # is a separate current-encumbrance dataset; insider-trading pledge
    # creation/release/invocation events remain in the NSE filing snapshot.
    try:
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=False, args=BROWSER_ARGS)
            ctx = browser.new_context(user_agent=USER_AGENT)
