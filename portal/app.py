...
def _ryb_watermark_svg() -> Markup:
    size=1000; pad=size*.047; ratio=1.1; avail=size-2*pad
    r=min(avail/(ratio+2), avail/(ratio*math.sqrt(3)/2+2)); s=r*ratio; h=s*math.sqrt(3)/2; ctv=h*2/3
    cx=size/2; cy=((pad+ctv+r)+(size-pad-ctv/2-r))/2
    bx,by=cx,cy-ctv; rx,ry=cx-s/2,cy+ctv/2; yx,yy=cx+s/2,cy+ctv/2
...
def inject_globals():
    return {"now_year":datetime.now(timezone.utc).year,"ryb_logo_svg":_ryb_logo_svg,"ryb_watermark_svg":_ryb_watermark_svg(),"css_version":_CSS_VERSION,"main_js_version":_MAIN_JS_VERSION,"scan_js_version":_SCAN_JS_VERSION}
...
def _band_label(pct:float)->str:
    a=abs(pct)
    return "within10" if a<=10 else "within20" if a<=20 else "within30" if a<=30 else "beyond30"
...
def scan_stock_detail(date_str,symbol):
    ...
    if full_df is not None and not full_df.empty and "Symbol" in full_df.columns:
        ...
        if not full_matches.empty:
            ...
            for field in ["ReportedBuyTxn","PricedValuePurchased","MissingValueQty","MissingValueTxn","MarketBuyValue","MarketSellValue","NetBuyValue","SellBuyRatioPct","HasMarketSell","SellBuyExclusion","DMA50","DMA200"]:
                if field in full_raw:raw[field]=full_raw[field]
